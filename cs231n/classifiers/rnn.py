...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'lstm':
      d_embed, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, cache_h)
    else:
      d_embed, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache_h)
  ...
  def sample(self, features, max_length=30):
    ...
    idx_word = np.ones((1, N), dtype=int) * self._start
    captions[:, 0] = idx_word
    for t in xrange(max_length):
      ...
      captions[:, t + 1] = idx_word

      pending_captions *= idx_word[:, 0] != self._end
      if not np.any(pending_captions):
        break
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
